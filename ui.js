...
function renderInv(){
    ...
    if(grid){grid.innerHTML='';
        ...
        cards.sort((a,b)=>{
            switch(invSortMode){
                case'rarity':return ri(AURAS.find(x=>x.id===b.aId)?.rarity||'common')-ri(AURAS.find(x=>x.id===a.aId)?.rarity||'common')||getAuraPower(b.aId,b.mod)-getAuraPower(a.aId,a.mod);
                case'name':return(AURAS.find(x=>x.id===a.aId)?.name||'').localeCompare(AURAS.find(x=>x.id===b.aId)?.name||'');
                case'count':return b.cnt-a.cnt||getAuraPower(b.aId,b.mod)-getAuraPower(a.aId,a.mod);
                default:return getAuraPower(b.aId,b.mod)-getAuraPower(a.aId,a.mod);
            }
        });
...
function renderEggs(){
    ...
    for(const e of EGG_TYPES){const ci=e.currency==='gold'?'🪙':'💎';const d=document.createElement('div');d.className='egg-shop-card';d.innerHTML=`<span class="egg-icon">${e.icon}</span><div class="egg-name">${e.name}</div><div class="egg-cost">${ci}${fmt(e.cost)}</div><div class="egg-desc">${Math.ceil(e.hatchTime/getEggSpd())}s</div><div style="display:flex;gap:3px;margin-top:4px"><button class="btn-sm" data-egg="${e.id}" data-bulk="0" style="font-size:.6rem;padding:1px 4px">Buy 1</button><button class="btn-sm" data-egg="${e.id}" data-bulk="1" style="font-size:.6rem;padding:1px 4px">Fill</button></div>`;sg.appendChild(d);}
    sg.querySelectorAll('.btn-sm[data-egg]').forEach(b=>b.addEventListener('click',()=>buyEgg(b.dataset.egg,b.dataset.bulk==='1')));
    // Expeditions
    renderExpeditions();
    // Auto-sell sync
...
function renderDungeons(){const l=document.getElementById('dungeon-list');l.innerHTML='';for(const d of DUNGEONS){const pw=getTotalPower(),ok=pw>=d.reqPower,cd=S.dungeonCds[d.id]||0,on=Date.now()<cd,rem=on?Math.ceil((cd-Date.now())/1000):0,done=S.dungeonsDone.includes(d.id);const remStr=on?`${Math.floor(rem/60)}:${(rem%60).toString().padStart(2,'0')}`:'';const el=document.createElement('div');el.className=`dungeon-card${ok?'':' locked'}`;el.innerHTML=`<div class="dungeon-info"><h4>${d.icon} ${d.name}${done?' ✅':''}</h4><p>${d.boss.name} (${fmt(d.boss.hp)}HP) | Multi-room run</p><div class="dungeon-req">Req:${fmt(d.reqPower)}</div></div><button class="dungeon-btn" ${!ok||on?'disabled':''} data-d="${d.id}">${on?`⏱️${remStr}`:ok?'⚔️ Enter':'🔒'}</button>`;l.appendChild(el);}l.querySelectorAll('.dungeon-btn').forEach(b=>b.addEventListener('click',()=>startDungeonRun(b.dataset.d)));}
...
function renderAutoSellCheckboxes(){
    ...
    container.innerHTML+=`<div style="display:flex;gap:4px;margin-bottom:6px;flex-wrap:wrap"><span style="font-size:.62rem;color:var(--dim);align-self:center">Quick:</span><button class="btn-sm" onclick="autoSellBelow('uncommon')" style="font-size:.58rem;padding:1px 4px">Below Uncommon</button><button class="btn-sm" onclick="autoSellBelow('rare')" style="font-size:.58rem;padding:1px 4px">Below Rare</button><button class="btn-sm" onclick="autoSellBelow('epic')" style="font-size:.58rem;padding:1px 4px">Below Epic</button><button class="btn-sm" onclick="autoSellBelow('legendary')" style="font-size:.58rem;padding:1px 4px">Below Legendary</button><button class="btn-sm" onclick="autoSellClearAll()" style="font-size:.58rem;padding:1px 4px">Clear All</button></div>`;
